...
mkdir -p "$PKG_DIR" "$RES_DIR/xml" "$RES_DIR/layout" "$RES_DIR/drawable" "$RES_DIR/raw"
...
# Make the words list available to the native widget so it can compute
# today's word without waiting for the Flutter app to open.
cp assets/words.json                          "$RES_DIR/raw/words.json"
